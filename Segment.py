...
class Segmentation:
    def __init__(self, model, loss, optimizer, epochs=200, device='cuda', save_path="./save_model/"):
    ...
    def Plot_train(self, loss, iou_train, iou_val):
    ...
    def fit(self, train_loader, val_loader):
        ...
        for i in tqdm(range(self.epochs)):
            ...
            avg_val_iou = self.evaluation(self.model, val_loader)
            ...
            if i%10==0:
#                 print('train loss: ', round(avg_train_loss.item(), 4), 
#                       'train acc: ', np.mean(avg_train_acc, axis=0), 
#                       'val acc: ', np.mean(avg_val_acc, axis=0))
                clear_output(wait=True)
                self.Plot_train(train_loss, train_iou, val_iou)
                if i>100:
                    torch.save(model.state_dict(), self.save_path+f'model_lr4_epochs_{i}')
